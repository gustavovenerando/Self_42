...
int placement_validation(char board[10][10], int x, int y){
    ...
    for(int pos = 0; pos < 10; pos++) {
        ...
        //Upper Diagonals
        int upper_y = y + pos;
        if(upper_y <= 9 && upper_x <= 9 && lower_x >= 0){
            if (board[upper_x][upper_y] == 'Q' || board[lower_x][upper_y] == 'Q')
                upper_diagonals = 0;
        }

        //Lower Diagonals
        int lower_y = y - pos;
        if(lower_y >= 0 && lower_x >= 0 && upper_x <= 9){
            if (board[lower_x][lower_y] == 'Q' || board[upper_x][lower_y] == 'Q')
                lower_diagonals = 0;
        }
    }
...
int main(){
    ...
    int can_place = placement_validation(chess_board, 6 , 3);
